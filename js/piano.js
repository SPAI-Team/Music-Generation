...
function isAccidental(note) {
  // used when building the piano to see which are the accidental notes
  let pc = note % 12;
  return pc === 1 || pc === 3 || pc === 6 || pc === 8 || pc === 10;
}

function buildKeyboard(container) {
  // dynamically generates the keyboard
  let nAccidentals = _.range(MIN_NOTE, MAX_NOTE + 1).filter(
    isAccidental
  ).length;
...
function buildNoteSequence(seed) {
  // NoteSequence is the way data is sent to the model
  return mm.sequences.quantizeNoteSequence(
    {
      ...
      notes: seed.map((n, idx) => ({
        // format of the input to the model
        pitch: n.note,
        startTime: idx * 0.5,
        endTime: (idx + 1) * 0.5,
      })),
...
function startSequenceGenerator(seed) {
  ...
  function generateNext() {
    // generates the notes based on the sequence
    if (!running) return;
...
function start() {
  ...
  document.body.style.backgroundSize = `25%, 30%`;
  document.body.style.backgroundPosition = `right 0px,left bottom`;
}
